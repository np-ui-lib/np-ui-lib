...
export class NpFileService {
    ...
    downloadCSVFile(data: any[], columns: any[], dateFormat: string, fileName?: string) {
    ...
    private createCSVData(data: any[], columnList: Column[], dateFormat: string) {
        ...
        for (const element of data) {
            ...
            for (const column of columnList) {
                if (element[column.dataField] !== undefined && element[column.dataField] !== null) {
                    if (column.dataType === DataTypes.Date) {
                        cols.push(this.datePipe.transform(element[column.dataField], dateFormat));
                    } else {
                        cols.push(`"${element[column.dataField].toString().replace(/\"/g, '""')}"`);
                    }
                }
                else {
                    cols.push('');
